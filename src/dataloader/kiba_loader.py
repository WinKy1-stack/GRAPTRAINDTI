"""
KIBA Dataset Loader
Kinase Inhibitor BioActivity dataset
"""
import os
import json
import pickle
...
class KIBADataset(Dataset):
    """
    KIBA Dataset for Drug-Target Interaction
    ...
    Reference:
        Tang et al. (2014) "Making Sense of Large-Scale Kinase Inhibitor Bioactivity Data Sets"
    """
    
    def __init__(self,
                 data_dir: str = "./data/kiba",
                 split: str = "train",
        ...
        """
        Args:
            data_dir: Path to KIBA data directory
        ...
        """
        super(KIBADataset, self).__init__()
        ...
        print(f"[INFO] KIBA {split.upper()} dataset loaded: {len(self.pairs)} pairs")
    ...
    def _load_data(self) -> Tuple[List[str], List[str], np.ndarray]:
        ...
        if not all([os.path.exists(ligands_file), 
            ...
            raise FileNotFoundError(
                f"KIBA data files not found in {self.data_dir}\n"
...
def get_kiba_dataloader(data_dir: str = "./data/kiba",
                        split: str = "train",
                        batch_size: int = 64,
                        num_workers: int = 4,
                        shuffle: bool = True,
    ...
    Args:
        data_dir: Path to KIBA data
        split: 'train', 'val', or 'test'
        batch_size: Batch size
        num_workers: Number of workers for data loading
        shuffle: Whether to shuffle data
        **kwargs: Additional arguments for KIBADataset
    
    Returns:
        DataLoader
    """
    dataset = KIBADataset(data_dir=data_dir, split=split, **kwargs)
...
def test_kiba_loader():
    """Test KIBA dataset loader"""
    print("\nTesting KIBA Dataset Loader...")
    print("=" * 60)
    
    # Note: This assumes KIBA data is available at ./data/kiba
    # If not, this test will fail
    
    try:
        dataloader = get_kiba_dataloader(
            data_dir="./data/kiba",
            split="train",
            batch_size=4,
            num_workers=0,
        ...
        print("Note: Make sure KIBA data is downloaded and placed in ./data/kiba/")
